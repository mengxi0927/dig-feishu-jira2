...
## 2026-09-22 代码部署完成，月度策略等待基线确认

- 已将 main 的 `f63fe0cc1b9f4e3ad943b6a99235662ade13cfa7` 构建为
  `dig-feishu-jira2:20260922-f63fe0c` 并替换正式容器，健康检查通过。
- 本地及服务器镜像均通过42项测试。
- 运行目录为 `releases/20260922-f63fe0c`；备份在
  `backups/20260922-f63fe0c/`（原配置、容器信息、切换前状态和基线预览）。
- 原容器已停止并保留为 `dig-feishu-jira2-sync-before-delta-20260922`，不得与新容器同时启动。
- 首次基线预览：正常工时表本年有效记录0条；Jira本年汇总8479条，全部为飞书缺失，
  无工时数值冲突。本期2026-08-22至2026-09-21共1099条汇总、7890小时。
- 未绕过基线校验、未写入历史补录。当前显式设置 `ASSIGNMENT_STRATEGY=legacy`，
  派工/报工原有每天06:00任务继续运行；新月度策略尚未启用。
- 等待确定初始历史数据的导入范围后，重新获取最新预览并建立基线，再切换
  `ASSIGNMENT_STRATEGY=monthly-delta`。不能直接把全部缺失历史视为本月补录。
